...
def generate_launch_description():
    # Get file path 
    param_file = os.path.join(get_package_share_directory('group_project'), 'config', 'mapper_params_online_async.yaml')
    map_root = os.path.join(get_package_share_directory('group_project'), 'maps')
    
    # Declare simulation time, true for Gazebo, false for real robots
    use_sim_time = LaunchConfiguration('use_sim_time')
    use_sim_time_arg = DeclareLaunchArgument('use_sim_time', default_value='false', description="Use Gazebo time")

    # Declare we want to resume map
    use_map_resume = LaunchConfiguration('use_map_file')
    use_map_resume_arg = DeclareLaunchArgument('use_map_file', default_value='True', description="Resume from existig map?")
    ...
    param_map_path_full = [map_root, '/', param_map_path]
